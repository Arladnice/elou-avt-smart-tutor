...
- **Коммиты делает только пользователь.** Агент не выполняет `git commit`. Вместо коммита — контрольная точка: показать `git status` и краткое резюме, дождаться подтверждения.
