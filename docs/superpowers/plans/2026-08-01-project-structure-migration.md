...
- [ ] `pytest tests -q` в `backend/` — не меньше 149 тестов, все зелёные
- [ ] `lint-imports` в `backend/` — 4 контракта соблюдены
- [ ] `ruff check .` в `backend/` — чисто
- [ ] Фронтенд: `npm run lint`, `npm run build`, `npm run check:fsd` — зелёные
- [ ] `docker compose up -d --build --wait` — оба контейнера healthy
- [ ] В прод-образе нет `telemetry_dataset.csv`, `*.pth` и скриптов обучения
- [ ] В корне репозитория 16 записей, ни одна из них — не служебный файл разработки
- [ ] `git log --follow` работает для перенесённых модулей
- [ ] Приёмочный сценарий в браузере пройден: пуск → дефект → парирование → ScoreCard
- [ ] Ветка `restructure` заархивирована тегом
