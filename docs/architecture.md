...
Порты стека (`8000` для API, `80` для интерфейса) зашиты в `docker-compose.yml`, а не берутся из переменных: подстановка `${...}` в compose-файле работает только с `.env` из каталога самого compose-файла, и ради двух чисел держать его в корне не стоило. Меняя их, поправьте и `FRONTEND_PORT`/`BACKEND_PORT` в `config.mk` — оттуда `make start` берёт адреса для вывода.
