...
Порт `80` намеренно не занимаем: на боевом хосте его вместе с `443` держит фронт-прокси (Caddy), который терминирует TLS, автоматически выпускает сертификат и проксирует запросы по имени домена на `8080`. Имена контейнеров закреплены (`tutor-backend`, `tutor-frontend`), чтобы команды диагностики не зависели от имени compose-проекта; имена **сервисов** при этом прежние — на них ссылаются `depends_on` и `frontend/nginx.conf`, они же остаются DNS-именами внутри сети.
