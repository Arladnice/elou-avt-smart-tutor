...
### Задачи
- 🔴 **ScoreCard для инструктора** — на панели инструктора (`InstructorPage.tsx`) добавить просмотр результатов сессии через `ScoreCard.tsx`.
- 🔴 **Объяснение Precision=2.7%** — в `evaluation_report.md` аргументировать дисбаланс классов (0.02% аварий) и 100% Recall для защиты перед жюри.
- 🔴 **Слайд «3 гипотезы кейса»** — в презентацию `ELOU_AVT_Smart_Tutor.pptx` добавить отдельный слайд с подтверждением гипотез №1, №2, №3 из `Кейс.pdf`.
- 🟠 **SessionTimeline в InstructorPage** — виджет таймлайна событий сессии оператора для дебрифинга.
- 🟠 **Замер WS-задержки** — измерить пинг WebSocket, задокументировать в `docs/performance.md`.
- 🟠 **Слайд «Обзор подходов»** — добавить в PPTX сравнение с DeltaSim/ИНИУС со ссылкой на Дозорцева.
- 🟡 **Human-in-the-Loop** — кнопка подтверждения оценки инструктором в интерфейсе.
- 🟡 **Freeze/Snapshot** — кнопка заморозки/паузы симуляции для инструктора.
