...
import { Modal, Tabs, Form, Input, InputNumber, Switch, Button, Select, message, Upload, Card, Popconfirm } from 'antd';
import { PlusOutlined, DeleteOutlined, UploadOutlined, FileTextOutlined, CodeOutlined } from '@ant-design/icons';
...
export const ScenarioBuilderModal: React.FC<ScenarioBuilderModalProps> = ({ visible, onClose }) => {
  const { reloadScenarios, scenarios } = useSimulator();
  ...
  const handleVisualSubmit = async (values: any) => {
    try {
      ...
      await apiService.createScenario(newScenario);
  ...
  const handleJsonSubmit = async () => {
    try {
      ...
      await apiService.importScenario(parsed);
