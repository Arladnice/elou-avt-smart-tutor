...
const AiAssistant: React.FC<AiAssistantProps> = ({ hideRiskTab = false, hideTrendTab = false }) => {
  ...
  const getAiMessage = () => {
    ...
    if (defects?.pump_fail) {
      return 'АВАРИЯ: Отказ сырьевого насоса Н-20! Прекращена подача сырья в печь. Снизьте уставки обеих печей, остановите Н-20 и затем закройте V-1.';
    }
  ...
  const suggestions = defects?.pump_fail
    ? ['Что делать при отказе Н-20?', 'Как снизить уставки обеих печей?', 'Как остановить Н-20?', 'Порядок действий по сценарию']
