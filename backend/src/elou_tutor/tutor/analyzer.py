...
class ErrorAnalyzer:
    ...
    def _evaluate_defect_handling(self, actions, defects_triggered,
        ...
        if "pump_fail" in defects_triggered:
            required_sequence = ("SP_DOWN", "SP3_DOWN", "N_20_STOP", "V1_CLOSE")
            try:
            ...
            if action_positions and action_positions == sorted(action_positions):
                return 100, [], [
                    ...
                    "Вы снизили уставки обеих печей, остановили Н-20 и только затем "
                    "перекрыли V-1, предотвратив сухой перегрев змеевиков.",
                    "Рекомендуемый следующий этап траектории: 'Прогар змеевика печи П-1'"
                ], "coil_overheat"
            return 30, [TECH_REGULATIONS["P1_DRY_HEAT"]], [
                "При отказе Н-20 сначала снизьте уставки П-1 и П-3, затем остановите Н-20 "
                "и только после этого закройте V-1, чтобы исключить сухой перегрев змеевиков.",
        ...
        if "power_fail" in defects_triggered:
            # Power fail сам снимает нагрев: модель переводит уставки обеих
            # печей в 20°C. Повторный SP_DOWN от оператора не требуется;
            # обязательна только явная отсечка подачи либо сброс давления.
            has_v1_close = "V1_CLOSE" in actions
            if has_v1_close or "V2_OPEN" in actions:
                return 100, [], [
        ...
        if "air_fail" in defects_triggered:
            ...
            has_both_setpoints_down = {"SP_DOWN", "SP3_DOWN"}.issubset(actions)
            # В рабочей сессии final_sensors всегда передаётся сервером.
            # Смена уставки — это только команда на остывание, поэтому до
            # достижения фактическими T-1/T-3 безопасного порога зачёт нельзя
            # выдавать. None оставлен для старых чистых unit-тестов анализатора.
            has_actual_cooling = final_sensors is None or (
                final_sensors.get("T_1", 999) <= limit_temp
                and final_sensors.get("T_3", 999) <= limit_temp
            )
            if has_both_setpoints_down and has_actual_cooling:
                return 100, [], [
                    "Поздравляем! Вы успешно отреагировали на отказ воздуха КИПиА (air_fail).",
                    "При потере управления пневмоклапанами вы снизили уставки обеих печей и дождались их охлаждения.",
            ...
            return 30, [{
                ...
                "text": "При падении давления воздуха КИПиА клапаны переходят в безопасное "
                        ...
                        "змеевиков необходимо снизить уставки П-1 и П-3 и дождаться фактического охлаждения печей."
            }], [
                "При отказе воздуха КИПиА снизьте уставки П-1 и П-3 ниже допустимой и дождитесь остывания обеих печей.",
                "Рекомендуемый адаптивный сценарий дообучения: 'Аварийный останов печей П-1 и П-3'"
            ], "shutdown"
        ...
        if "k2_pump_fail" in defects_triggered:
            ...
            if has_esd or required.issubset(actions):
                return 100, [], [
                    ...
                    "закрыли V-1, не допустив заполнения куба К-2 и захлёбывания колонны.",
                    "Квалификационная траектория успешно завершена! Вы готовы к итоговому экзамену.",
                ], "startup"
            ...
            return 30, [{
                ...
                "text": "При отказе насосов Н-4/Н-32 вывод кубового остатка прекращается, "
                        ...
                        "по факту отказа насосов, а не по уровнемеру: закрыть V-3, "
                        "остановить Н-20, закрыть V-1 либо активировать ПАЗ (ESD).",
            }], [
                "При отказе насосов откачки К-2 закройте дренаж V-3, остановите Н-20 и "
                "закройте V-1, прекратив подачу в переполняющийся куб.",
        ...
        if "elou_desalt_fail" in defects_triggered:
            ...
            return 30, [{
                ...
                "text": "При отказе электрообессоливания солесодержание поднимается с 4,2 до "
                        ...
                        "ЭЛОУ, остановить Н-20, сохранить циркуляцию К-1, снизить нагрев и дождаться охлаждения печей.",
            }], [
                "При проскоке солей и воды не восстанавливайте подачу в неисправный ЭЛОУ: "
                "остановите Н-20, закройте V-1, включите горячую циркуляцию и дождитесь T-1/T-3 не выше 200°C.",
