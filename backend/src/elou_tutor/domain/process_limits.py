...
# Порог 15% принят для строки ПАЗ «К1 (куб)» в согласованной таблице.
# Отдельные каналы LRCSA 602, LRSA 602A и LRSA 602B в физической модели пока
# не разделяются: она передаёт единое значение уровня куба.
COLUMN_LEVEL_LOW_INTERLOCK = 15.0 # % (ПАЗ по низкому уровню куба К-1)
...
# Пороги ниже соответствуют согласованной ПАЗ-таблице. Уровневая защита К2
# показана как 2oo3 (LRCSA 604, LRSA 604A, LRSA 604B); в физической модели
# пока используется единое значение уровня, без моделирования отказов каналов.
